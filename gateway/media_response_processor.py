"""Incremental transformation of MEDIA directives in streamed responses."""
...
class MediaStreamBuffer:
    ...
    def _complete_media_end(self) -> Optional[int]:
        ...
        # Prevent the regex end-of-string branch from accepting a partial path.
...
class MediaResponseProcessor:
    """Replace MEDIA directives through an async path callback.

    Normal text is returned immediately without invoking the callback. The
    buffer retains only a possible ``MEDIA:`` prefix or an incomplete media
    path. Set ``intercept_stream`` to false when only complete responses should
    be transformed.
    """
    ...
    def buffers_stream(self) -> bool:
        """Whether streaming deltas pass through the MEDIA boundary buffer."""
        return self._buffer is not None

    async def render(self, text: str) -> str:
        """Transform complete text when it contains a MEDIA directive."""
    ...
    async def feed(self, chunk: str) -> str:
        """Return the portion of a stream delta that is safe to emit now."""
        if self._buffer is None:
            return chunk
        ready = self._buffer.feed(chunk)
        return await self.render(ready)

    async def finish(self, authoritative_text: Optional[str] = None) -> str:
        """Flush the withheld stream tail when the producer completes."""
        if self._buffer is None:
            return ""
        tail = self._buffer.finish(authoritative_text)
        return await self.render(tail)
